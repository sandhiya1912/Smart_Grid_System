...
class SmartGrid {
    ...
    // Dijkstra algorithm to find the shortest path from src to dest
    public int dijkstra(int src, int dest, int[] parent) {
        // Create a priority queue for distances
        PriorityQueue<int[]> pq = new PriorityQueue<>(Comparator.comparingInt(a -> a[1]));
        
        // Distance array to store shortest distance from src to i
        int[] dist = new int[places];
        Arrays.fill(dist, Integer.MAX_VALUE);
        
        // Initialize parent array for tracking path
        ...
        while (!pq.isEmpty()) {
            ...
            // If this path is longer than the known shortest path, skip it
            if (distance > dist[u]) {
                continue;
            }
            
            // Visit all neighbors of u
            for (Container neigh : Adjlist.get(u)) {
                int v = neigh.destination;
                ...
                if (dist[u] != Integer.MAX_VALUE && dist[u] + weight < dist[v]) {
                    ...
                    // Update parent to reconstruct path
                    parent[v] = u;
    ...
    // Method to find the path using Dijkstra's algorithm and store voltage info
    public int First_findpath(int src, int dest, int p_src) {
        int[] parent = new int[places];
        int shortestDistance = dijkstra(src, dest, parent);
        
        if (shortestDistance == Integer.MAX_VALUE) {
            return 0; // No path found
        }
        
        // Reconstruct the path from dest to src using parent array
        List<Integer> pathList = new ArrayList<>();
        for (int at = dest; at != -1; at = parent[at]) {
            pathList.add(at);
        }
        Collections.reverse(pathList); // Reverse to get path from src to dest
        
        // Store path and voltage info
        int pathIndex = 1;
        for (int i = 0; i < pathList.size(); i++) {
            int node = pathList.get(i);
            if (i > 0) {
                int prevNode = pathList.get(i-1);
                // Find the edge to get voltage
                for (Container neigh : Adjlist.get(prevNode)) {
                    if (neigh.destination == node) {
                        voltpath[pathIndex-1] = neigh.voltage;
                        break;
                    }
                }
            }
    ...
    // Method to find the path distance using Dijkstra's algorithm
    public int Second_findpath(int src, int dest) {
        int[] parent = new int[places];
        return dijkstra(src, dest, parent);
    }
    ...
    public void GridOptimization(int s, int d, int dist) {
        ...
        if (sum > min_distance) {
            ...
            // Since we're using Dijkstra, we need to find the edge to remove
            // We know the path from 0 to d is stored in the path array
            for (int i = 1; i < places; i++) {
                if (path[i] != 0 && path[i-1] != 0) {
                    removeEdge(path[i-1], path[i]);
                }
            }
